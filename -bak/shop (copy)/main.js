let main = {
    ...
    test: function(){
        ...
        setTimeout(() => {
            ...
            for(let i = 0; i < utils.randomInt(0, 20); i++){
                let o = {category: testCategory2};
                if(!utils.randomInt(0, 4)){
                    o = {
                        title: utils.generateRandomChars(utils.randomInt(3, 5), testChars),
                        description: utils.generateRandomChars(utils.randomInt(7, 23), testChars),
...
let productManager = {
    ...
    addCategory: function(o){
        let category = this.dom.cloneableCategory.cloneNode(true);
            category.setAttribute('id', 'cat-' + utils.generateRandomChars());
    ...
    addProduct: function(o){
        ...
        o.category.appendChild(product);
        o.category.classList.remove('hidden');
...
let utils = {
    ...
    randomCharacters: 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456789!@#$%^&*()_-+'.split(''),
